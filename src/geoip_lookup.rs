...
use crate::{error::EchoIpError, model::GeoInfo};
...
impl GeoipLookup {
    ...
    pub fn lookup_geo_for_ip(&self, _ip: IpAddr) -> Result<GeoInfo, EchoIpError> {
        let geoip_city: geoip2::City =
            self.city_reader.lookup::<geoip2::City>(_ip).map_err(|err| EchoIpError::MaxMindDbFailed { source: err })?;
        let geoip_asn: geoip2::Asn =
            self.asn_reader.lookup::<geoip2::Asn>(_ip).map_err(|err| EchoIpError::MaxMindDbFailed { source: err })?;
        ...
        let city = geoip_city.city.unwrap().names.unwrap().get("en").unwrap().to_string();
